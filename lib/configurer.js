	...
	function isReadable() {
		return new Promise(resolve => {
			if (configReadable === null) {
				fs.access(path.resolve(configObject.filePath), fs.constants.R_OK, err => {
	...
	function isWritable() {
		return new Promise(resolve => {
			if (configWritable === null) {
				fs.access(path.resolve(configObject.filePath), fs.constants.W_OK, err => {
	...
	function isDirectoryWritable() {
		return new Promise(resolve => {
			if (directoryWritable === null) {
				fs.access(path.dirname(path.resolve(configObject.filePath)), fs.constants.W_OK, err => {
			...
			isWritable().then(writable => {
				if (writable) {
					fs.writeFile(path.resolve(configObject.filePath),
						JSON.stringify(configObject, null, 2), 'utf8', err => {
							if (err) {
